...
def load_and_cache_gen_data(args, filename, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    ...
    if os.path.exists(cache_fn) and not is_sample:
        ...
        if pool:
            ...
            features = convert_examples_to_features(tqdm(tuple_examples, total=len(tuple_examples)))
...
def prepare_obfuscation(examples):
    def _is_number(t):
        pass
...
def load_and_cache_defect_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    ...
    if not os.path.exists(os.path.dirname(cache_fn)):
        os.makedirs(os.path.dirname(cache_fn))
    if os.path.exists(filename + '.examples'):
        examples = pickle.load(open(filename + '.examples', mode='rb'))
    else:
        examples = read_examples(filename, args.data_num, args.task)
        # pickle.dump(examples, open(filename + '.examples', mode='wb'))
        # calc_stats(examples, tokenizer, is_tokenize=True)

    # obfuscated_examples = prepare_obfuscation(examples)
    if os.path.exists(cache_fn):
        logger.info("Load cache data from %s", cache_fn)
        data = torch.load(cache_fn)
    else:
        ...
        # all_source_ids = torch.tensor([f.source_ids for fs in features for f in fs], dtype=torch.long)
        # all_labels = torch.tensor([f.label for fs in features for f in fs], dtype=torch.long)
        # all_corrupt_labels = torch.tensor([f.corrupt_label for fs in features for f in fs], dtype=torch.long)
        if 'train' in filename:
            all_source_ids = torch.tensor([f.source_ids for fs in features for f in fs], dtype=torch.long)
            all_labels = torch.tensor([f.label for fs in features for f in fs], dtype=torch.long)
            all_corrupt_labels = torch.tensor([f.corrupt_label for fs in features for f in fs], dtype=torch.long)
        else:
            all_source_ids = torch.tensor([fs[0].source_ids for fs in features], dtype=torch.long)
            all_labels = torch.tensor([fs[0].label for fs in features], dtype=torch.long)
            all_corrupt_labels = torch.tensor([fs[0].corrupt_label for fs in features], dtype=torch.long)
        data = TensorDataset(all_source_ids, all_labels, all_corrupt_labels)
